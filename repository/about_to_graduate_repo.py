"""
Database operations for about to graduate alert tracker
"""
...
TABLES = [
    """
    CREATE TABLE IF NOT EXISTS about_to_graduate_settings (
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS about_to_graduate_channels (
        id INT AUTO_INCREMENT PRIMARY KEY,
        guild_id BIGINT NOT NULL,
...
async def setup_about_to_graduate_tables() -> bool:
    """Create about to graduate alert tables if they don't exist"""
    success = True
    ...
    for table_query in TABLES:
        try:
            ...
            logger.error(f"Error creating about to graduate alert table: {e}")
            success = False
...
async def get_enabled_guild_channels() -> Dict[int, List[int]]:
    ...
    FROM about_to_graduate_channels tc
    JOIN about_to_graduate_settings ts ON tc.guild_id = ts.guild_id
...
async def get_guild_settings(guild_id: int) -> Dict:
    """Get about to graduate alert settings for a guild"""
    query = """
    SELECT id, guild_id, enabled, created_at, updated_at 
    FROM about_to_graduate_settings 
    WHERE guild_id = %s
    """
    ...
    if not result:
        ...
        insert_query = """
        INSERT INTO about_to_graduate_settings 
        (guild_id, enabled) 
        VALUES (%s, %s)
        """
...
async def update_guild_settings(guild_id: int, settings: Dict) -> bool:
    """Update about to graduate alert settings for a guild"""
    ...
    query = f"""
    UPDATE about_to_graduate_settings 
...
async def get_channels(guild_id: int) -> List[int]:
    """Get channels configured for about to graduate alert tracking in a guild"""
    query = "SELECT channel_id FROM about_to_graduate_channels WHERE guild_id = %s"
